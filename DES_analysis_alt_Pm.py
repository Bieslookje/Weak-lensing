...
class power_spectrum(cosmology):
    ...
    def get_Pmm_interp(self):    
        ...
        P_1h_sub = np.load('halo_model_power_spectra_substructure.npz')['P_1h_total'] 
        k = np.load('halo_model_power_spectra.npz')['k']
        z = np.load('halo_model_power_spectra.npz')['z']
        PK = P_2h + P_1h_sub
        Pmm = RegularGridInterpolator((z, k), PK, bounds_error=False, fill_value=None)
        return Pmm
