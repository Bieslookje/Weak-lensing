...
class subhalo_observables(subhalo_properties):
    ...
    def density_parameters(self, evolved=True):
        """
        Subhalo density profile parameters binned by mass.

        -----
        Input
        -----
        evolved : bool, optional
            If True, use evolved subhalo mass m0.
            If False, use unevolved infall mass ma200.
        nbins : int, optional
        ...
        ------
        m   : array
            Bin-centered subhalo mass [Msun]
        rs  : array
            Weighted mean scale radius r_s in each mass bin
        ct  : array
            Weighted mean tidal truncation parameter c_t in each mass bin
        """

        mass = self.m0 if evolved else self.ma200
        lnm = np.log(mass)

        # Histogram of counts (weighted)
        N, lnm_edges = np.histogram(lnm, weights=self.weight, bins=100)

        # Compute weighted histograms
        rs_sum, _ = np.histogram(lnm, weights= self.weight*self.rs0, bins=lnm_edges)
        ct_sum, _ = np.histogram(lnm, weights= self.weight*self.ct0, bins=lnm_edges)

        # Weighted averages
        rs = np.full_like(rs_sum, np.nan, dtype=float)
        nonzero = N > 0
        rs[nonzero] = rs_sum[nonzero] / N[nonzero]

        ct = np.full_like(ct_sum, np.nan, dtype=float)
        ct[nonzero] = ct_sum[nonzero] / N[nonzero]

        # Bin centers
        lnm = (lnm_edges[1:] + lnm_edges[:-1]) / 2.
        m = np.exp(lnm)

        return m / self.Msun, rs/self.Mpc , ct
