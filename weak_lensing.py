...
class cosmology(units_and_constants):
    ...
    def __init__(self):
        ...
        self.dc0 = (3./20.)*(12.*np.pi)**(2./3.) # delta_c = ~1.686' EdS linear collapse threshold
        self.Dv0 = 18.*np.pi**2                  # Delta_v = ~178, EdS halo virial overdensity
    ...
    def Dv_BryanNorman(self,Om_mz:float) -> float:
        '''
    ...
    def peak_height(self, sigmaM):
        '''
        Calculate peak-height (nu) 
        '''
        dc = self.dc_NakamuraSuto(self.OmegaM)
        nu = dc/sigmaM
        return nu
